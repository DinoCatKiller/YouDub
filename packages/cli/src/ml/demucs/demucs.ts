...
export class Demucs {
  ...
  constructor(
      ...
      private options?: { executionProvider?: 'cpu' | 'webgpu' | 'cuda' },
  ) {}
  private ort: any;
  ...
  async load() {
    ...
    if (ep === 'cuda') {
      this.ort = await import('onnxruntime-node-gpu');
    } else {
  ...
  private async _separateBuffer(input: Float32Array, sampleRate: number): Promise<DemucsStems> {
    ...
    for (let seg = 0; seg < numSegments; seg++) {
      ...
      const inputTensor = new this.ort.Tensor('float32', segBuf, [1, NUM_CHANNELS, SEGMENT_LEN]);
