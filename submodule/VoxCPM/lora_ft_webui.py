...
def detect_sample_rate(pretrained_path: str) -> Optional[int]:
    """Read audio_vae_config.sample_rate from the model's vocab_2567.txt.
    ...
    """
    config_file = os.path.join(pretrained_path, "vocab_2567.txt")
...
def start_training(
    ...
    # Auto-detect sample_rate from model vocab_2567.txt to prevent mismatch
    ...
    if detected_sr is not None:
        if int(sample_rate) != detected_sr:
            training_log += (
                ...
                f"(read from {pretrained_path}/vocab_2567.txt audio_vae_config.sample_rate)\n"
            )
